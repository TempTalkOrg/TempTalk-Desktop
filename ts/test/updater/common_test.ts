  ...
  describe('#getUpdateFileName', () => {
    it('successfully gets version', async () => {
      ...
      assert.strictEqual(
        ...
        'yelling-desktop-win-1.23.2.exe'
      );
      assert.strictEqual(
        getUpdateFileName(mac, platform(), arch),
        'yelling-desktop-mac-1.23.2.zip'
      );
      assert.strictEqual(
        getUpdateFileName(windowsBeta, platform(), arch),
        'yelling-desktop-beta-win-1.23.2-beta.1.exe'
      );
      assert.strictEqual(
        getUpdateFileName(macBeta, platform(), arch),
        'yelling-desktop-beta-mac-1.23.2-beta.1.zip'
      );
    });
  });
});
