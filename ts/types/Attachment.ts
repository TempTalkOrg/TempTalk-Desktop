...
export const isEncryptedVoiceMessage = (attachment: Attachment): boolean => {
  return isVoiceMessage(attachment) && Boolean(attachment.encryptionKey);
};
...
export const save = ({
  ...
  const isObjectURLRequired = isUndefined(attachment.path);
  const url = !isUndefined(attachment.path)
...
export const getSuggestedFilename = ({
  ...
  if (attachment.fileName) {
    return sanitize(attachment.fileName);
  }

  const prefix = 'tt-attachment';
