...
>(function MediaDeviceSelect(
  ...
  const mergedProps = React.useMemo(
    () => mergeProps(props, { className }, { className: 'media-device-list' }),
    [className, props]
  );
  ...
  return (
    <ul ref={ref} {...mergedProps}>
      {devices.map((device, index) => (
        ...
        >
          <div
            className="media-device-select-item-text"
            onClick={() => handleActiveDeviceChange(device.deviceId)}
          >
            {device.label}
          </div>
          <div className="media-device-select-item-status">
            {isActive(device.deviceId, activeDeviceId, index) ? (
              <IconMediaDeviceCheck />
            ) : null}
          </div>
        </li>
      ))}
