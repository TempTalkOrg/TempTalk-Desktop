...
function genUrlMessage(appId: string, url: string) {
  const message = `${appId}--${url}`;
  return Buffer.from(message);
}

export async function generateUrlSig(
  privateKey: BinaryType | string,
  appId: string,
  url: string
) {
  let keyBuffer;
  if (typeof privateKey === 'string') {
    keyBuffer = await loadHexFromPath(privateKey);
  } else {
    keyBuffer = privateKey;
  }

  const message = genUrlMessage(appId, url);
  return binaryToHex(sign(keyBuffer, message));
}

export function verifyUrlSig(
  publicKey: BinaryType,
  appId: string,
  url: string,
  sigHex: string
  ...
  const message = genUrlMessage(appId, url);
  return verify(publicKey, message, hexToBinary(sigHex));
}
