...
(function () {
  ...
  Whisper.RecorderView = Whisper.View.extend({
    ...
    initialize() {
      ...
      this.interval = setInterval(this.onDurationChange.bind(this), 1000);
    ...
    checkTimeLimit(duration) {
      ...
      if (delta < 0) {
        this.$('.record-time-tip').hide();
        this.finish();
        return;
      }

      if (delta <= 10) {
        this.$('.record-time-tip').show();
        this.$('.record-time-tip').text(
          window.i18n('recordTimeLimitTip', [delta])
        );
      }
    },
    updateTime(duration) {
